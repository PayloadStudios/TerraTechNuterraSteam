...
namespace Nuterra.BlockInjector
{
    internal static class DirectoryBlockLoader
    {
		...
		public static void CreateJSONBlock(FileInfo Json, bool l = false)
		{
			L("Get locals for " + Json.Name, l);
			JObject jObject = JObject.Parse(StripComments(File.ReadAllText(Json.FullName)));
			CreateJSONBlock(Json.Name, Json.FullName, jObject, l);
		}

		public static void CreateJSONBlock(string name, string filePath, JObject jObject, GameObject target = null, bool l = false)
        {
            try
            {
                ...
                {
                    string otherName = ExistingJSONBlock.Name;//BlockLoader.CustomBlocks[overlap].Name;
                    Console.WriteLine("Could not read block " + name + "\n\nBlock ID collides with " + otherName);
                    BlockLoader.Timer.Log($" ! Could not read #{name} - Block ID collides with { otherName }!");
                    return;
                }
                ...
                blockbuilder._mcb.FilePath = filePath;
            ...
            {
                Console.WriteLine("Could not read block " + name + "\n at " + filePath + "\n\n" + E);
                BlockLoader.Timer.Log($" ! Could not read #{name} - \"{E.Message}\"");
            }
        }
